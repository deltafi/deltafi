...
RSpec.describe 'Auth API' do
  ...
  def should_allow(dn, url)
    ...
    expect(last_response.headers.has_key?('X-User-ID')).to be(true)
  end
  ...
  def create_user(params)
    post '/users', body=params.to_json
  ...
  def update_user(id:, updates:)
    put "/users/#{id}", body=updates.to_json
  end
  ...
  describe "/auth" do
    it "should deny access (400) if required request headers are not set" do
      get '/auth', params={}, rack_env={ "HTTP_SSL_CLIENT_SUBJECT_DN" => 'CN=TestUser1' }
      expect(last_response.status).to eq(400)
      expect(last_response.body).to eq("400 Bad Request: Missing required header: X_ORIGINAL_URL")
      get '/auth', params={}, rack_env={ "HTTP_X_ORIGINAL_URL" => 'https://test.deltafi.org' }
      expect(last_response.status).to eq(400)
    ...
    it 'should deny access (401) an unknown user' do
