...
const emit = defineEmits(["metadataChanged"]);
...
const loadPairsFromModelValue = () => {
  if (props.modelValue === null || props.modelValue === "") {
    ...
    pairs.value = props.modelValue
      ...
      .map((i) => i.split(":"))
      .map((j) => {
        return { key: j[0].trim(), value: j[1].trim() };
      });
  }
};
...
const emitUpdate = () => {
  if (pairs.value.length == 0) {
    emit("metadataChanged", null);
  } else {
    emit("metadataChanged", pairs.value.map((p) => `${p.key}: ${p.value}`).join(", "));
  }
};
