...
const { enableTestOnErrorDataSourceFlowByName, disableTestOnErrorDataSourceFlowByName } = useDataSource();
...
const confirmationPopup = (event, name, testMode) => {
  if (testMode) {
    confirm.require({
      ...
      message: `Disable Test Mode for the <b>${name}</b> data source?`,
    ...
    confirm.require({
      target: event.currentTarget,
      group: `${rowData.value.flowType}_testmode_${rowData.value.name}`,
      message: `Enable Test Mode for the <b>${name}</b> data source?`,
...
const toggleFlowState = async (flowName, newFlowTestMode) => {
  if (!newFlowTestMode) {
    notify.info("Enabling Test Mode", `Enabling Test Mode for the <b>${flowName}</b> data source.`, 3000);
    await enableTestOnErrorDataSourceFlowByName(flowName);
  } else {
    notify.info("Disabling Test Mode", `Disabling Test Mode for the <b>${flowName}</b> data source.`, 3000);
    await disableTestOnErrorDataSourceFlowByName(flowName);
  }
