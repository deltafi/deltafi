...
const { enableTestTimedDataSourceFlowByName, disableTestTimedDataSourceFlowByName } = useDataSource();
...
const toggleFlowState = async (flowName, newFlowTestMode) => {
  if (!newFlowTestMode) {
    ...
    await enableTestTimedDataSourceFlowByName(flowName);
  } else {
    notify.info("Disabling Test Mode", `Disabling Test Mode for the <b>${flowName}</b> data source.`, 3000);
    await disableTestTimedDataSourceFlowByName(flowName);
  }
