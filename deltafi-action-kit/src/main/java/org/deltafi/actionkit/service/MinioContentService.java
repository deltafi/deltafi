...
@ApplicationScoped
@IfBuildProfile("prod")
public class MinioContentService implements ContentService {
    ...
    public ObjectReference putObject(ObjectReference objectReference, byte[] object) {
        try {
            InputStream dataInputStream = new ByteArrayInputStream(object);
            ObjectWriteResponse objectWriteResponse = minioClient.putObject(
                    PutObjectArgs.builder()
                            .bucket(objectReference.getBucket())
                            .object(objectReference.getName())
                            .stream(dataInputStream, object.length, -1)
                            .build()
            );

            return fromObjectWriteResponse(objectWriteResponse, object.length);
