...
// PluginNameToPackageName converts a kebab-case plugin name to a snake_case Python package name
...
// ActionNameToFileName converts an action name to a lowercase snake_case Python file name
// Handles kebab-case, PascalCase, and mixed case
// Examples: "my-new-action" -> "my_new_action", "Super" -> "super", "SuperDuper" -> "super_duper"
func ActionNameToFileName(actionName string) string {
	...
	result := strings.ReplaceAll(actionName, "-", "_")

	// Build result with underscores before uppercase letters (except at start)
	var builder strings.Builder
	for i, r := range result {
		if unicode.IsUpper(r) {
			// Insert underscore before uppercase letter (except at start)
			if i > 0 && builder.Len() > 0 {
				// Check if previous character was not already an underscore
				prevChar := rune(result[i-1])
				if prevChar != '_' {
					builder.WriteRune('_')
				}
			}
			...
			// Lowercase letter, digit, or underscore - keep as-is
			builder.WriteRune(r)
		}
	}
	...
	// Handle edge case: if result is empty, return original in lowercase
	if result == "" {
