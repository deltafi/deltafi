...
import static org.deltafi.test.asserters.ActionResultAssertions.assertFormatResult;

class MergeContentFormatActionTest {
    private final MergeContentFormatAction action = new MergeContentFormatAction();
    ...
    void mergesContentToBinaryConcatenation() {
        ...
        assertFormatResult(resultType)
                .formattedContentMatches(actionContent -> binaryContentMatches(actionContent, expectedOutput));
    }
    ...
    private ResultType runAction(ArchiveType archiveType) {
        return action.format(runner.actionContext(), new MergeContentParameters(archiveType),
                input("thing1.txt", "thing2.txt"));
    }

    private FormatInput input(String... files) {
        return FormatInput.builder().content(runner.saveContentFromResource(files)).build();
    }

    private void verifyResult(ArchiveType archiveType, ResultType resultType) {
        assertFormatResult(resultType)
                .formattedContentMatches(actionContent -> contentMatches(actionContent,
