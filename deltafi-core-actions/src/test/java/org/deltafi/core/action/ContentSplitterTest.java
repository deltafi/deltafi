...
public class ContentSplitterTest {
    // TODO: redo
/*
    ContentSplitter action = new ContentSplitter();
    DeltaFiTestRunner runner = DeltaFiTestRunner.setup(action, "SplitterLoadActionTest");

    @Test
    void testSplitOneResult() {
        TransformInput transformInput = TransformInput.builder().content(runner.saveContentFromResource("content1")).build();
        ReinjectParameters reinjectParameters =  new ReinjectParameters("testFlow");

        ResultType result = action.transform(runner.actionContext(), reinjectParameters, transformInput);
