...
@Component
@Slf4j
public class FilterByCriteria extends TransformAction<FilterByCriteriaParameters> {
    public FilterByCriteria() {
        super("""
                The FilterByCriteria action allows you to filter or pass DeltaFiles based on specific criteria defined using Spring Expression Language (SpEL). The action takes a list of SpEL expressions that are evaluated against the metadata and content. Depending on the configured filter behavior, the action filters if 'ANY', 'ALL', or 'NONE' of the expressions match.
                Examples:
