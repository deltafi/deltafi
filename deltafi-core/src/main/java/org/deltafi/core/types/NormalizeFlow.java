...
@Slf4j
public class NormalizeFlow extends Flow {
    ...
    public List<DeltaFiConfiguration> findByConfigType(ConfigType configType)  {
        return switch (configType) {
            case NORMALIZE_FLOW -> List.of(asFlowConfiguration());
    ...
    public DeltaFiConfiguration asFlowConfiguration() {
        NormalizeFlowConfiguration normalizeFlowConfiguration = new NormalizeFlowConfiguration(name);
        normalizeFlowConfiguration.setTransformActions(transformActions.stream().map(ActionConfiguration::getName).toList());
        if (loadAction != null) {
            normalizeFlowConfiguration.setLoadAction(loadAction.getName());
        }
        return normalizeFlowConfiguration;
    }
}
