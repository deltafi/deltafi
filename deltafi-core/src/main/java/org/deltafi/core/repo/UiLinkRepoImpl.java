...
public class UiLinkRepoImpl implements UiLinkRepoCustom {

    private static final Map<String, Index> INDICES = Map.of(
            "unique_name", new Index().named("unique_name").on("name", Sort.Direction.ASC).on("linkType", Direction.ASC).unique());

    private final MongoTemplate mongoTemplate;

    @Override
    public void ensureAllIndices() {
        IndexOperations idxOps = mongoTemplate.indexOps(Link.class);
