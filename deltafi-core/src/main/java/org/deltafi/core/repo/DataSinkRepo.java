...
@Repository
@Transactional
public interface DataSinkRepo extends FlowRepo, DataSinkRepoCustom {
    @Modifying
    @Query(value = "UPDATE flows SET expected_annotations = cast(:expectedAnnotations AS jsonb) WHERE name = :flowName AND type = 'DATA_SINK'",
            nativeQuery = true)
