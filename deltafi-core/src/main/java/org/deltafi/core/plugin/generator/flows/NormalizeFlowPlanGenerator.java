...
@Service
public class NormalizeFlowPlanGenerator {
     ...
     */
    public List<FlowPlan> generateNormalizeFlowPlans(String baseFlowName, List<ActionGeneratorInput> transformActions, List<ActionGeneratorInput> loadActions) {
        ...
        if (loadActionConfigs.size() == 1) {
            flowPlans.add(generateNormalizeFlowPlan(planName, transformActionConfigs, loadActionConfigs.get(0)));
        } else {
            int i = 1;
            for (LoadActionConfiguration loadAction : loadActionConfigs) {
                flowPlans.add(generateNormalizeFlowPlan(planName + "-" + i++, transformActionConfigs, loadAction));
            }
        }
    ...
    private FlowPlan generateNormalizeFlowPlan(String planName, List<TransformActionConfiguration> transformActionConfigs, LoadActionConfiguration loadAction) {
        NormalizeFlowPlan normalizeFlowPlan = new NormalizeFlowPlan(planName, "Sample ingress flow");

        normalizeFlowPlan.setTransformActions(transformActionConfigs);
        normalizeFlowPlan.setLoadAction(loadAction);
