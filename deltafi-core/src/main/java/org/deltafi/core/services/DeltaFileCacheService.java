...
@Service
public class DeltaFileCacheService {
    ...
    public DeltaFileCacheService(DeltaFileRepo deltaFileRepo, DeltaFiPropertiesService deltaFiPropertiesService) {
    ...
    public void removeOlderThan(int seconds) {
        deltaFileCache.values().stream()
                ...
                .map(d -> deltaFileCache.remove(d.getDid()))
                .forEach(d -> {
