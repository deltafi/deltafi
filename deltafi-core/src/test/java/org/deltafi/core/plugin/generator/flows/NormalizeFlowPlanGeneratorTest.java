...
class NormalizeFlowPlanGeneratorTest {

    NormalizeFlowPlanGenerator normalizeFlowPlanGenerator = new NormalizeFlowPlanGenerator();

    @Test
    void testDefaults() {
        List<FlowPlan> flowPlans = normalizeFlowPlanGenerator.generateNormalizeFlowPlans("base", null, null);
    ...
    void testMultipleLoadActions() {
        ...
        List<FlowPlan> flowPlans = normalizeFlowPlanGenerator.generateNormalizeFlowPlans("base", transforms, loads);
    ...
    void matches(FlowPlan flowPlan, String name, List<ActionConfigMatcher> transformActionMatchers, ActionConfigMatcher loadActionMatcher) {
        if (flowPlan instanceof NormalizeFlowPlan normalizeFlowPlan) {
            assertThat(normalizeFlowPlan.getName()).isEqualTo(name);
            assertThat(normalizeFlowPlan.getDescription()).isEqualTo("Sample ingress flow");
            List<TransformActionConfiguration> transformActions = normalizeFlowPlan.getTransformActions();
            ...
            assertThat(normalizeFlowPlan.getLoadAction()).matches(loadActionMatcher);
        } else {
